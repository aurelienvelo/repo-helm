...
{{- $email := .email | default "" }}
{{- $secretObj := dict "username" .username "password" .password "auth" $auth }}
{{- if $email }}
  {{- $_ := set $secretObj "email" $email }}
{{- end }}
{{- $dockerConfig := dict "auths" (dict .registry $secretObj) }}
{{- printf "%s" (toJson $dockerConfig | b64enc) }}
{{- end }}
{{- else }}
{{- printf "{}" | b64enc }}
{{- end }}
